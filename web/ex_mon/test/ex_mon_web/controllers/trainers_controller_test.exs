defmodule ExMonWeb.Controllers.TrainersControllerTest do
  ...
  describe "show/2" do
    setup %{conn: conn} do
      params = %{name: "Levi", password: "123456"}
    ...
    test "when the is a trainer with the given id, returns the trainer", %{conn: conn} do
      params = %{name: "Levi", password: "123456"}
      ...
      assert %{"id" => _id, "inserted_at" => _inserted_at, "name" => "Levi", "pokemon" => []} =
               response
    end

    test "when the is an error, returns the error", %{conn: conn} do
      response =
        conn
